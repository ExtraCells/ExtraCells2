    ...
    public boolean onActivate(EntityPlayer player, Vec3 pos) {
		...
		if(locked && s != null && mon != null){
			...
			if(FluidUtil.isFilled(s2)){
				...
				IAEFluidStack not = mon.injectItems(fl.copy(), Actionable.SIMULATE, new MachineSource(this));
				...
				if(extract != null){
					...
					if(empty1.left == 0){
						mon.injectItems(FluidUtil.createAEFluidStack(new FluidStack(fluid, (int) extract.getStackSize())), Actionable.MODULATE, new MachineSource(this));
						return true;
					}
					ItemStack empty = empty1.right;
					if(empty != null){
