...
public class GuiHandler implements IGuiHandler {
	...
	private static Object getContainer(int ID, EntityPlayer player,
			Object[] args) {
		switch (ID) {
		case 0:
		...
		case 3:
			...
			return new ContainerFluidStorage(fluidInventory3, player,
					storageCell);
		default:
			return null;
		}
	...
	public static Object getGui(int ID, EntityPlayer player) {
		...
		case 0:
			return new GuiFluidStorage(player,
					"extracells.part.fluid.terminal.name");
		case 1:
			return new GuiFluidStorage(player,
					"extracells.part.fluid.terminal.name");
		case 3:
			return new GuiFluidStorage(player,
					"extracells.item.storage.fluid.portable.name");
		default:
			return null;
		}
	}
	...
	private static Object getPartContainer(ForgeDirection side,
			EntityPlayer player, World world, int x, int y, int z) {
		PartECBase part = (PartECBase) ((IPartHost) world
				.getTileEntity(x, y, z)).getPart(side);
		return part.getServerGuiElement(player);
	}

	public static Object getPartGui(ForgeDirection side, EntityPlayer player,
	...
	public static void launchGui(int ID, EntityPlayer player, World world,
			int x, int y, int z) {
	...
	public Object getClientGuiElement(int ID, EntityPlayer player, World world,
		...
		if (world.getBlock(x, y, z) == BlockEnum.FLUIDCRAFTER.getBlock()) {
			...
			if (tileEntity == null
					|| !(tileEntity instanceof TileEntityFluidCrafter))
				return null;
			return new GuiFluidCrafter(player.inventory,
					((TileEntityFluidCrafter) tileEntity).getInventory());
		}
		if (world != null
			...
			if (tileEntity instanceof TileEntityFluidInterface)
				return new GuiFluidInterface(player,
						(IFluidInterface) tileEntity);
			else if (tileEntity instanceof TileEntityFluidFiller)
				return new GuiFluidFiller(player,
	...
	public Object getServerGuiElement(int ID, EntityPlayer player, World world,
		...
		ForgeDirection side = ForgeDirection.getOrientation(ID);
		if (world != null
				&& world.getBlock(x, y, z) == BlockEnum.FLUIDCRAFTER.getBlock()) {
			TileEntity tileEntity = world.getTileEntity(x, y, z);
			if (tileEntity == null
					|| !(tileEntity instanceof TileEntityFluidCrafter))
				return null;
		...
		if (world != null
			...
			if (tileEntity == null)
				return null;
			if (tileEntity instanceof TileEntityFluidInterface)
				return new ContainerFluidInterface(player,
						(IFluidInterface) tileEntity);
			else if (tileEntity instanceof TileEntityFluidFiller)
				return new ContainerFluidFiller(player.inventory,
						(TileEntityFluidFiller) tileEntity);
			return null;
		}
		if (world != null && side != ForgeDirection.UNKNOWN)
