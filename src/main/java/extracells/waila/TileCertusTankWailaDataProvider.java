...
public class TileCertusTankWailaDataProvider implements IWailaDataProvider {
	...
	public List<String> getWailaBody(ItemStack itemStack,
			List<String> list, IWailaDataAccessor accessor,
			IWailaConfigHandler config) {
		...
		if(tag.hasKey("fluidID")){
			...
			if(fluidID == -1){
				...
				list.add(StatCollector.translateToLocal("extracells.tooltip.amount") + ": 0mB / " + tag.getInteger("maxFluid") + "mB");
				return list;
			}else{
		...
		if(tag.hasKey("maxFluid") && tag.hasKey("currentFluid"))
			list.add(StatCollector.translateToLocal("extracells.tooltip.amount") + ": "  + tag.getInteger("currentFluid") + "mB / " + tag.getInteger("maxFluid") + "mB");
		return list;
	}
	...
	public NBTTagCompound getNBTData(EntityPlayerMP player, TileEntity tile,
		...
		if(tile instanceof TileEntityCertusTank){
			if(((TileEntityCertusTank) tile).tank.getFluid() == null)
				...
				tag.setInteger("fluidID", ((TileEntityCertusTank) tile).tank.getFluid().fluidID);
				tag.setInteger("currentFluid", ((TileEntityCertusTank) tile).tank.getFluidAmount());
			}
			tag.setInteger("maxFluid", ((TileEntityCertusTank) tile).tank.getCapacity());
		}
		return tag;
	}
