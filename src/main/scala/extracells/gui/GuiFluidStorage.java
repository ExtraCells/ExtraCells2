...
public class GuiFluidStorage extends GuiContainer implements IFluidSelectorGui {
        ...
        if (!this.containerFluidStorage.getFluidStackList().isEmpty()) {
            outerLoop: for (int y = 0; y < 4; y++) {
                for (int x = 0; x < 9; x++) {
                    ...
                    if (0 <= widgetIndex && widgetIndex < listSize) {
                        AbstractFluidWidget widget = tmp.get(widgetIndex);
            ...
            for (int x = 0; x < 9; x++) {
                for (int y = 0; y < 4; y++) {
                    int widgetIndex = y * 9 + x + this.currentScroll * 9;
                    if (0 <= widgetIndex && widgetIndex < listSize) {
                        tmp.get(widgetIndex).drawTooltip(x * 18 + 7, y * 18 - 1, mouseX, mouseY);
                    } else {
                        break;
                    }
                }
    ...
    public void updateSelectedFluid() {
        ...
        for (IAEFluidStack stack : this.containerFluidStorage.getFluidStackList()) {
            if (stack.getFluid() == this.containerFluidStorage.getSelectedFluid()) this.currentFluid = stack;
        }
    }
}
