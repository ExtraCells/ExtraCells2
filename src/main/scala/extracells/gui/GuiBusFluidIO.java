...
public class GuiBusFluidIO extends ECGuiContainer
    ...
    public void actionPerformed(GuiButton button) {
        ...
        new PacketBusFluidIO(this.player, (byte) button.id, this.part).sendPacketToServer();
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float alpha, int mouseX, int mouseY) {
        ...
        if (this.hasNetworkTool) drawTexturedModalRect(this.guiLeft + 179, this.guiTop + 93, 178, 93, 68, 68);
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        for (byte i = 0; i < 9; i++) {
            ...
            if (!overlayRendered && this.fluidSlotList.get(i).canRender())
                overlayRendered = renderOverlay(this.fluidSlotList.get(i), mouseX, mouseY);
        }

        for (Object button : this.buttonList) {
            if (button instanceof WidgetRedstoneModes) ((WidgetRedstoneModes) button)
                    .drawTooltip(mouseX, mouseY, (this.width - this.xSize) / 2, (this.height - this.ySize) / 2);
        }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseBtn) {
        ...
        for (WidgetFluidSlot fluidSlot : this.fluidSlotList) {
            if (isPointInRegion(fluidSlot.getPosX(), fluidSlot.getPosY(), 18, 18, mouseX, mouseY)) {
                if ((part instanceof PartGasImport || part instanceof PartGasExport)
    ...
    public boolean renderOverlay(WidgetFluidSlot fluidSlot, int mouseX, int mouseY) {
        if (isPointInRegion(fluidSlot.getPosX(), fluidSlot.getPosY(), 18, 18, mouseX, mouseY)) {
            ...
            drawGradientRect(
                    fluidSlot.getPosX() + 1,
                    fluidSlot.getPosY() + 1,
                    fluidSlot.getPosX() + 17,
                    fluidSlot.getPosY() + 17,
                    -0x7F000001,
                    -0x7F000001);
            GL11.glEnable(GL11.GL_LIGHTING);
    ...
    public void setRedstoneControlled(boolean _redstoneControlled) {
        ...
        if (this.redstoneControlled) this.buttonList
                .add(new WidgetRedstoneModes(0, this.guiLeft - 18, this.guiTop, 16, 16, this.part.getRedstoneMode()));
    }
