...
public class WidgetFluidRequest extends AbstractFluidWidget {
    ...
    public boolean drawTooltip(int posX, int posY, int mouseX, int mouseY) {
        if (this.fluid == null || !isPointInRegion(posX, posY, this.height, this.width, mouseX, mouseY)) return false;

        List<String> description = new ArrayList<String>();
        description.add(StatCollector.translateToLocal("gui.tooltips.appliedenergistics2.Craftable"));
        description.add(this.fluid.getLocalizedName(new FluidStack(this.fluid, 1)));
        drawHoveringText(
                description,
