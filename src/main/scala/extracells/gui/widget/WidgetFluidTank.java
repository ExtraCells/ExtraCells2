...
public class WidgetFluidTank extends Gui {
    ...
    public void draw(int guiX, int guiY, int mouseX, int mouseY) {
        if (this.tank == null || 73 < 31) return;
        ...
        if (fluid != null && fluid.amount > 0) {
            ...
            GL11.glColor3f(
                    (fluid.getFluid().getColor() >> 16 & 0xFF) / 255.0F,
                    (fluid.getFluid().getColor() >> 8 & 0xFF) / 255.0F,
                    (fluid.getFluid().getColor() & 0xFF) / 255.0F);
            if (iconHeightRemainder > 0) {
                drawTexturedModelRectFromIcon(this.posX + 1, this.posY + 2, fluidIcon, 16, iconHeightRemainder);
            }
    ...
    public void drawDirectionTooltip(int x, int y) {
        ...
        description.add(StatCollector.translateToLocal("extracells.tooltip.direction." + this.direction.ordinal()));

        if (this.tank == null || this.tank.getFluid() == null) {
            description.add(StatCollector.translateToLocal("extracells.tooltip.empty1"));
        } else {
            if (this.tank.getFluid().amount > 0 && this.tank.getFluid().getFluid() != null) {
