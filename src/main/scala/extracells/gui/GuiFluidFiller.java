...
public class GuiFluidFiller extends GuiContainer {
    ...
    public GuiFluidFiller(EntityPlayer player, TileEntityFluidFiller tileentity) {
        ...
        this.player = player;
        this.fluidContainerSlot = new WidgetSlotFluidContainer(player, tileentity, 80, 35);
    }
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        this.fontRendererObj.drawString(
                StatCollector.translateToLocal("extracells.block.fluidfiller.name").replace("ME ", ""),
                5,
                5,
                0x000000);
        int i = this.fluidContainerSlot.getPosX();
        int j = this.fluidContainerSlot.getPosY();
        if (GuiUtil.isPointInRegion(this.guiLeft, this.guiTop, i, j, 16, 16, mouseX, mouseY)) {
        ...
        } else this.fluidContainerSlot.drawWidget();
    }

    @Override
    protected void mouseClicked(int mouseX, int mouseY, int mouseBtn) {
        super.mouseClicked(mouseX, mouseY, mouseBtn);
        if (this.fluidContainerSlot != null) if (GuiUtil.isPointInRegion(
                this.guiLeft,
                this.guiTop,
                this.fluidContainerSlot.getPosX(),
                this.fluidContainerSlot.getPosY(),
                18,
                18,
                mouseX,
                mouseY)) {
                    this.fluidContainerSlot.mouseClicked(this.player.inventory.getItemStack());
                }
    }
}
