...
public class ECFluidGridBlock implements IGridBlock {
    ...
    public ItemStack getMachineRepresentation() {
        ...
        return new ItemStack(
                loc.getWorld().getBlock(loc.x, loc.y, loc.z), 1, loc.getWorld().getBlockMetadata(loc.x, loc.y, loc.z));
    }
    ...
    public final void setNetworkStatus(IGrid _grid, int _usedChannels) {
        if (this.grid != null && this.host instanceof IListenerTile && this.grid != _grid) {
            ...
            if (this.host instanceof TileEntityFluidFiller && this.grid.getCache(IStorageGrid.class) != null)
                ((TileEntityFluidFiller) this.host)
                        .postChange(
                                ((IStorageGrid) this.grid.getCache(IStorageGrid.class)).getFluidInventory(),
                                null,
                                null);
        } else {
            this.grid = _grid;
