...
public class PartFluidExport extends PartFluidIO {
    ...
    public TickRateModulation doWork(int rate, int TicksSinceLastCall) {
        if (!isActive() || tile == null) return TickRateModulation.IDLE;
        IFluidHandler facingTank = getFacingTank();
        if (facingTank == null) return TickRateModulation.IDLE;
        ...
        for (Fluid fluid : filter) {
            if (fluid != null) {
                IAEFluidStack stack = extractFluid(
                        AEApi.instance().storage().createFluidStack(new FluidStack(fluid, rate * TicksSinceLastCall)),
                        Actionable.SIMULATE);
                ...
                if (filled > 0) {
                    ...
                    if (stack != null && stack.getStackSize() > 0) {
                        ...
                        if (actuallyFilled < stack.getStackSize()) {
                            ...
                            IAEFluidStack returned = injectFluid(
                                    AEApi.instance().storage().createFluidStack(new FluidStack(fluid, toReturn)),
                                    Actionable.MODULATE);
                            if (returned != null) {
                                FMLLog.severe(
                                        "[ExtraCells2] Export bus at %d:%d,%d,%d voided %d mL of %s",
