...
public class PartDrive extends PartECBase implements ICellContainer, IInventoryUpdateReceiver {
    ...
    public void blinkCell(int slot) {
        if (slot > 0 && slot < this.blinkTimers.length) this.blinkTimers[slot] = 15;
    }
    ...
    public boolean readFromStream(ByteBuf data) throws IOException {
        ...
        for (int i = 0; i < this.cellStatuses.length; i++) this.cellStatuses[i] = data.readByte();
        return true;
    }
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 3; j++) {
                if (this.cellStatuses[j + i * 3] > 0) {
                    ...
                    else rh.setBounds(3, 12 - j * 3, 14, 8, 10 - j * 3, 16);
