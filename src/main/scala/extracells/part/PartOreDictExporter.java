...
public class PartOreDictExporter extends PartECBase implements IGridTickable {
    ...
    private static class OreListMatcher implements Predicate<AEItemStack> {
        ...
        public OreListMatcher(ArrayList<ItemStack> input) {
            for (ItemStack is : input) ores.add(AEItemStack.create(is));
        }
    ...
    public void updateFilter() {
        ...
        if (matcher != null && !this.filter.contains("@") && !this.filter.contains("~")) {
            ...
            for (String name : OreDictionary.getOreNames())
                for (ItemStack s : OreDictionary.getOres(name)) if (matcher.test(s)) filtered.add(s);
            filterPredicate = new OreListMatcher(filtered);
        } else filterPredicate = null;
    }
    ...
    public boolean doWork(int rate, int ticksSinceLastCall) {
        ...
        if (this.filterPredicate != null) {
            ...
            for (IAEItemStack stack : items) {
                if (stack == null || !this.filterPredicate.test((AEItemStack) stack)) continue;
    ...
    public List<String> getWailaBodey(NBTTagCompound data, List<String> list) {
        ...
        else list.add(StatCollector.translateToLocal("extracells.tooltip.oredict") + ":");
        return list;
    }
    ...
    public final TickRateModulation tickingRequest(IGridNode node, int TicksSinceLastCall) {
        if (isActive()) return doWork(10, TicksSinceLastCall) ? TickRateModulation.FASTER : TickRateModulation.SLOWER;
