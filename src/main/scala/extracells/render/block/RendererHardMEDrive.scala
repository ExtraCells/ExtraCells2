...
object RendererHardMEDrive extends ISimpleBlockRenderingHandler {
  ...
  val tex =
    new ResourceLocation("extracells", "textures/blocks/hardmedrive.png")
  ...
  override def renderInventoryBlock(
    ...
    renderer.renderFaceYNeg(
      ...
      0.0d,
      0.0d,
      renderer.getBlockIconFromSideAndMetadata(block, 0, 3)
    )
    tessellator.draw
    tessellator.startDrawingQuads
    tessellator.setNormal(0.0f, 1.0f, 0.0f)
    renderer.renderFaceYPos(
      ...
      0.0d,
      0.0d,
      renderer.getBlockIconFromSideAndMetadata(block, 1, 3)
    )
    tessellator.draw
    tessellator.startDrawingQuads
    ...
    renderer.renderFaceZPos(
      ...
      0.0d,
      0.0d,
      renderer.getBlockIconFromSideAndMetadata(block, 3, 3)
    )
    tessellator.draw
    ...
    renderer.renderFaceXNeg(
      ...
      0.0d,
      0.0d,
      renderer.getBlockIconFromSideAndMetadata(block, 4, 3)
    )
    tessellator.draw
    tessellator.startDrawingQuads
    tessellator.setNormal(1.0f, 0.0f, 0.0f)
    renderer.renderFaceXPos(
      ...
      0.0d,
      0.0d,
      renderer.getBlockIconFromSideAndMetadata(block, 5, 3)
    )
    tessellator.draw
    GL11.glTranslatef(0.5f, 0.5f, 0.5f)
  }
    ...
    meta match {
      case 2 =>
        renderZNeg(
          renderer,
          block,
          generateRenderInformations(tileEntityHardMeDrive)
        )
      case 3 =>
        renderZPos(
          renderer,
  ...
  def renderXPos(
      renderer: RenderBlocks,
      block: Block,
      renderInformations: Array[RenderInformation]
  ) {
    val tessellator = Tessellator.instance
  ...
  protected class Icon(minU: Float, maxU: Float, minV: Float, maxV: Float)
      extends IIcon {
