...
public class HandlerItemStorageFluid implements
		IMEInventoryHandler<IAEFluidStack>, IHandlerFluidStorage {
	...
	public HandlerItemStorageFluid(ItemStack _storageStack,
		...
		this.totalTypes = ((IFluidStorageCell) _storageStack.getItem())
				.getMaxTypes(_storageStack);
		this.totalBytes = ((IFluidStorageCell) _storageStack.getItem())
				.getMaxBytes(_storageStack) * 250;

		for (int i = 0; i < this.totalTypes; i++)
			this.fluidStacks.add(FluidStack.loadFluidStackFromNBT(this.stackTag
					.getCompoundTag("Fluid#" + i)));
	...
	public boolean canAccept(IAEFluidStack input) {
		...
		for (FluidStack fluidStack : this.fluidStacks) {
			if (fluidStack == null
					|| fluidStack.getFluid().getID() == input.getFluid()
							.getID())
		...
		for (int i = 0; i < this.fluidStacks.size(); i++) {
			...
			if (currentStack != null
				...
				if (endAmount >= 0) {
					...
					removedStack = AEApi.instance().storage()
							.createFluidStack(currentStack.copy());
					if (mode == Actionable.MODULATE) {
		...
		for (int i = 0; i < currentFluids.size(); i++) {
			...
			if (notAdded != null && currentStack == null) {
				if (input.getStackSize() <= freeBytes()) {
					...
					FluidStack toWrite = new FluidStack(notAdded.getFluid()
							.getID(), freeBytes());
					currentFluids.set(i, toWrite);
					if (mode == Actionable.MODULATE) {
						writeFluidToSlot(i, toWrite);
