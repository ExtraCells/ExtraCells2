...
object MekanismGas {
  ...
  def init {
    ...
    api.registerNewLayer(
      classOf[LayerGasHandler].getName,
      classOf[IGasHandler].getName
    )
    api.registerNewLayer(
      classOf[LayerTubeConnection].getName,
      classOf[ITubeConnection].getName
    )
  ...
  def postInit {
    ...
    while (it.hasNext) {
      ...
      if (
        (!FluidRegistry.isFluidRegistered(fluid)) && FluidRegistry
          .registerFluid(fluid)
      )
        fluidGas += (g -> fluid)
    }
