...
public class DriverFluidInterface implements SidedBlock {
    ...
    public ManagedEnvironment createEnvironment(World world, int x, int y, int z, ForgeDirection side) {
        TileEntity tile = world.getTileEntity(x, y, z);
        if (tile == null || (!(tile instanceof IPartHost || tile instanceof IFluidInterface))) return null;
    ...
    private static PartFluidInterface getFluidInterface(World world, int x, int y, int z, ForgeDirection dir) {
        ...
        if (dir == null || dir == ForgeDirection.UNKNOWN) {
            for (ForgeDirection side : ForgeDirection.VALID_DIRECTIONS) {
                ...
                if (part != null && part instanceof PartFluidInterface) return (PartFluidInterface) part;
            }
            return null;
        } else {
    ...
    public class Environment extends ManagedEnvironment implements NamedBlock {
        ...
        @Callback(
                doc =
                        "function(side:number):table -- Get the configuration of the fluid interface on the specified direction.")
        public Object[] getFluidInterfaceConfiguration(Context context, Arguments args) {
            ...
            if (part == null) return new Object[] {null, "no interface"};
        ...
        @Callback(
                doc =
                        "function(side:number[, database:address, entry:number]):boolean -- Configure the filter in fluid interface on the specified direction.")
        public Object[] setFluidInterfaceConfiguration(Context context, Arguments args) {
            ForgeDirection dir = ForgeDirection.getOrientation(args.checkInteger(0));
            if (dir == null || dir == ForgeDirection.UNKNOWN) return new Object[] {null, "unknown side"};
            IFluidInterface part = tile instanceof IFluidInterface
                    ? (IFluidInterface) tile
                    : getFluidInterface(tile.getWorldObj(), tile.xCoord, tile.yCoord, tile.zCoord, dir);
            ...
            try {
                ...
                if (data == null) part.setFilter(dir, null);
                else {
    ...
    static class Provider implements EnvironmentProvider {
        ...
        public Class<? extends li.cil.oc.api.network.Environment> getEnvironment(ItemStack stack) {
            ...
            if (stack.getItem() == Item.getItemFromBlock(BlockEnum.ECBASEBLOCK.getBlock())
                    && stack.getItemDamage() == 0) return Environment.class;
            return null;
        }
    }
}
