...
object WirelessHandlerUpgradeAE extends IWirelessTermHandler {
  ...
  override def canHandle(itemStack: ItemStack): Boolean = {
    ...
    (OCUtils.isRobot(itemStack) && OCUtils.getComponent(
      new RobotData(itemStack),
      ItemOCUpgrade
    ) != null) ||
    (OCUtils.isDrone(itemStack) && OCUtils.getComponent(
      new DroneData(itemStack),
      ItemOCUpgrade
    ) != null)
  }
