...
class UpgradeAE(host: EnvironmentHost)
    extends ManagedEnvironment
    with appeng.NetworkControl[TileSecurity]
    with ec.NetworkControl[TileSecurity] {
  ...
  @Callback(doc =
    "function([number:amount]):number -- Transfer selecte fluid to your ae system."
  )
  def sendFluids(context: Context, args: Arguments): Array[AnyRef] = {
    ...
    val notInjectet = inv.injectItems(
  ...
  @Callback(doc =
    "function():boolean -- Return true if the card is linket to your ae network."
  )
