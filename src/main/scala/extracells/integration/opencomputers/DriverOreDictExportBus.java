...
public class DriverOreDictExportBus implements SidedBlock {
    ...
    private static PartOreDictExporter getExportBus(World world, int x, int y, int z, ForgeDirection dir) {
        ...
        if (dir == null || dir == ForgeDirection.UNKNOWN) {
            for (ForgeDirection side : ForgeDirection.VALID_DIRECTIONS) {
                ...
                if (part instanceof PartOreDictExporter) return (PartOreDictExporter) part;
            }
            return null;
        } else {
    ...
    public static class Environment extends ManagedEnvironment implements NamedBlock {
        ...
        @Callback(
                doc =
                        "function(side:number):string -- Get the configuration of the ore dict export bus pointing in the specified direction.")
        ...
        @Callback(
                doc =
                        "function(side:number[, filter:string]):boolean -- Set the configuration of the ore dict export bus pointing in the specified direction.")
    ...
    static class Provider implements EnvironmentProvider {
        ...
        public Class<? extends li.cil.oc.api.network.Environment> getEnvironment(ItemStack stack) {
            if (stack.getItem() == ItemEnum.PARTITEM.getItem()
                    && stack.getItemDamage() == PartEnum.OREDICTEXPORTBUS.ordinal()) return Environment.class;
            return null;
        }
    }
}
