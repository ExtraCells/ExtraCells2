...
/** This class is meant to be copied to your own mod which implements IGW-Mod.
  ...
  * mod loading stage, this will notify the player when it doesn't have IGW in
  * the instance. It also needs to have the config option enabled to notify the
  * player. This config option will be generated in its own config file.
  * @author
  *   MineMaarten https://github.com/MineMaarten/IGW-mod
  */
object IGWSupportNotifier {
  private val LATEST_DL_URL: String =
    "http://minecraft.curseforge.com/mc-mods/223815-in-game-wiki-mod/files/latest"
  ...
  if (
    FMLCommonHandler.instance.getSide == Side.CLIENT && !Loader.isModLoaded(
      "IGWMod"
    )
  ) {
  ...
  def onPlayerJoin(event: TickEvent.PlayerTickEvent) {
    if (
      event.player.worldObj.isRemote && event.player == FMLClientHandler.instance.getClientPlayerEntity
    ) {
  ...
  private class CommandDownloadIGW extends CommandBase {
    ...
    def processCommand(
        p_71515_1_ : ICommandSender,
        p_71515_2_ : Array[String]
    ) {
      ...
      try {
        if (Minecraft.getMinecraft.thePlayer != null)
          Minecraft.getMinecraft.thePlayer.addChatMessage(
            new ChatComponentText("Downloading IGW-Mod...")
          )
        ...
        case e: Throwable => {
          ...
          if (Minecraft.getMinecraft.thePlayer != null)
            Minecraft.getMinecraft.thePlayer.addChatComponentMessage(
              new ChatComponentText(
                EnumChatFormatting.RED + "Failed to download"
              )
            )
          try {
            finalize
