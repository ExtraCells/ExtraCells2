...
object IGW {
  ...
  def init {
    for (item <- ItemEnum.values()) {
      if (item != ItemEnum.CRAFTINGPATTERN && item != ItemEnum.FLUIDITEM) {
        if (item == ItemEnum.FLUIDPATTERN) {
          WikiRegistry.registerBlockAndItemPageEntry(
            item.getSizedStack(1),
            item.getSizedStack(1).getUnlocalizedName.replace(".", "/")
          )
        } else if (
          item == ItemEnum.STORAGECOMPONENT || item == ItemEnum.STORAGECASING
        ) {
          ...
          for (sub <- list) {
            ...
            WikiRegistry.registerBlockAndItemPageEntry(
              stack,
              stack.getUnlocalizedName.replace(".", "/")
            )
          }
        }
      }
    ...
    for (block <- BlockEnum.values()) {
      ...
      val list = new util.ArrayList[java.lang.Object]
      Item
        .getItemFromBlock(block.getBlock)
        .getSubItems(
          Item.getItemFromBlock(block.getBlock),
          Extracells.ModTab,
          list
        )
      for (sub <- list) {
        ...
        WikiRegistry.registerBlockAndItemPageEntry(
          stack,
          stack.getUnlocalizedName.replace(".", "/").replace("tile/", "")
        )
      }
    }
  }
