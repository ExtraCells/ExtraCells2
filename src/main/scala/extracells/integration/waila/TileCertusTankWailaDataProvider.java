...
public class TileCertusTankWailaDataProvider implements IWailaDataProvider {
    ...
    public NBTTagCompound getNBTData(
        ...
        if (tile instanceof TileEntityCertusTank) {
            if (((TileEntityCertusTank) tile).tank.getFluid() == null) tag.setInteger("fluidID", -1);
            else {
                tag.setInteger(
                        "fluidID", ((TileEntityCertusTank) tile).tank.getFluid().getFluidID());
    ...
    public List<String> getWailaBody(
            ItemStack itemStack, List<String> list, IWailaDataAccessor accessor, IWailaConfigHandler config) {
