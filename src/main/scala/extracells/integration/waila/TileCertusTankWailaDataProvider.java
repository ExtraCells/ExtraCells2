    ...
    public NBTTagCompound getNBTData(EntityPlayerMP player, TileEntity tile, NBTTagCompound tag, World world, int x,
        ...
        if (tile instanceof TileEntityCertusTank) {
            if (((TileEntityCertusTank) tile).tank.getFluid() == null) tag.setInteger("fluidID", -1);
                ...
                tag.setInteger("fluidID", ((TileEntityCertusTank) tile).tank.getFluid().getFluidID());
    ...
    public List<String> getWailaBody(ItemStack itemStack, List<String> list, IWailaDataAccessor accessor,
        ...
        if (tag.hasKey("fluidID")) {
            ...
            if (fluidID == -1) {
                ...
                list.add(
                        StatCollector.translateToLocal("extracells.tooltip.amount") + ": 0mB / "
                                + tag.getInteger("maxFluid")
                                + "mB");
                return list;
            } else {
        ...
        if (tag.hasKey("maxFluid") && tag.hasKey("currentFluid")) list.add(
