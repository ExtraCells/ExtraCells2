...
class UniversalTerminalRecipe extends TemplateRecipeHandler {
  ...
  override def loadCraftingRecipes(result: ItemStack) {
    if (
      result != null && result.getItem == ItemEnum.UNIVERSALTERMINAL.getItem
    ) {
      val cachedRecipe = new CachedShapelessRecipe(true)
      cachedRecipe.computeVisuals
      arecipes.add(cachedRecipe)
  ...
  override def hasOverlay(
    ...
    (super.hasOverlay(gui, container, recipe)) || ((this.isRecipe2x2(
      recipe
    )) && (RecipeInfo.hasDefaultOverlay(gui, "crafting2x2")))

  override def getOverlayRenderer(
      gui: GuiContainer,
      recipe: Int
  ): IRecipeOverlayRenderer = {
    ...
    val positioner: IStackPositioner =
      RecipeInfo.getStackPositioner(gui, "crafting2x2")
    if (positioner == null) {
      return null
    }
  ...
  override def getOverlayHandler(
      gui: GuiContainer,
      recipe: Int
  ): IOverlayHandler = {
  ...
  private class CachedShapelessRecipe(isUniversal: Boolean)
      extends CachedRecipe {
    private val ingredients = new util.ArrayList[PositionedStack]
    private val result: PositionedStack = new PositionedStack(
      ItemEnum.UNIVERSALTERMINAL.getDamagedStack(0),
      119,
      24
    )
    setIngredients
    ...
    override def getIngredients: util.List[PositionedStack] = {
      this.getCycledIngredients(
        UniversalTerminalRecipe.this.cycleticks / 20,
        this.ingredients
      )
    }
    ...
    def setIngredients() {
      ...
      val stack2: PositionedStack =
        new PositionedStack(UniversalTerminal.terminals, 43, 6, false)
      stack2.setMaxSize(1)
