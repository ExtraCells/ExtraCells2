...
object ItemOCUpgrade extends ItemECBase with UpgradeItemAEBase {
  ...
  override def getUnlocalizedName =
    super.getUnlocalizedName.replace("item.extracells", "extracells.item")
  ...
  override def getSubItems(
      item: Item,
      tab: CreativeTabs,
      list: java.util.List[_]
  ) {
    def add[T](list: java.util.List[T], value: Any) =
      list.add(value.asInstanceOf[T])
