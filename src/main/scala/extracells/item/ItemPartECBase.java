...
public class ItemPartECBase extends Item implements IPartItem, IItemGroup {
    ...
    public IPart createPartFromItemStack(ItemStack itemStack) {
        try {
            return PartEnum.values()[MathHelper.clamp_int(itemStack.getItemDamage(), 0, PartEnum.values().length - 1)]
    ...
    public void getSubItems(Item item, CreativeTabs creativeTab, List itemList) {
        for (int i = 0; i < PartEnum.values().length; i++) {
            ...
            if (part.getMod() == null || part.getMod().isEnabled()) itemList.add(new ItemStack(item, 1, i));
        }
    }
