...
public class ItemFluidPattern extends ItemECBase {
    ...
    public static ItemStack getPatternForFluid(Fluid fluid) {
        ...
        if (fluid != null) itemStack.getTagCompound().setString("fluidID", fluid.getName());
        return itemStack;
    }
    ...
    public IIcon getIcon(ItemStack itemStack, int pass) {
        if (pass == 0) {
            ...
            if (fluid != null) return fluid.getIcon();
        }
        return this.icon;
    }
    ...
    public String getItemStackDisplayName(ItemStack itemStack) {
        Fluid fluid = getFluid(itemStack);
        if (fluid == null) return StatCollector.translateToLocal(getUnlocalizedName(itemStack));
    ...
    public ItemStack onItemRightClick(ItemStack itemStack, World world, EntityPlayer entityPlayer) {
        if (entityPlayer.isSneaking()) return ItemEnum.FLUIDPATTERN.getSizedStack(itemStack.stackSize);
        return itemStack;
    }
