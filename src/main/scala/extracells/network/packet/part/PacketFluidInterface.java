        ...
        if (p.openContainer != null && p.openContainer instanceof ContainerFluidInterface) {
            ...
            if (Minecraft.getMinecraft().currentScreen != null
                ...
                for (int i = 0; i < this.filter.length; i++) {
                    if (gui.filter[i] != null) gui.filter[i].setFluid(FluidRegistry.getFluid(this.filter[i]));
                }
            }
        }
    }
            ...
            case 0:
                ...
                for (int i = 0; i < this.tank.length; i++) {
                    if (tag.hasKey("tank#" + i))
                        this.tank[i] = FluidStack.loadFluidStackFromNBT(tag.getCompoundTag("tank#" + i));
                    else this.tank[i] = null;
                }
                this.filter = new Integer[tag.getInteger("lengthFilter")];
                for (int i = 0; i < this.filter.length; i++) {
                    if (tag.hasKey("filter#" + i)) this.filter[i] = tag.getInteger("filter#" + i);
                    else this.filter[i] = -1;
                }
                break;
            case 1:
    ...
    public void writeData(ByteBuf out) {
        switch (this.mode) {
            case 0:
                ...
                for (int i = 0; i < this.tank.length; i++) {
                    if (this.tank[i] != null) {
                        tag.setTag("tank#" + i, this.tank[i].writeToNBT(new NBTTagCompound()));
                    }
                }
