...
public class TileEntityFluidFiller extends TileBase implements IActionHost, ICraftingProvider, IECTileEntity,
    ...
    public void cellUpdate(MENetworkCellArrayUpdate event) {
        ...
        if (storage != null) postChange(storage.getFluidInventory(), null, null);
    }
    ...
    public void postChange(IBaseMonitor<IAEFluidStack> monitor, Iterable<IAEFluidStack> change,
        ...
        for (IAEFluidStack fluid : ((IMEMonitor<IAEFluidStack>) monitor).getStorageList()) {
            if (!oldFluids.contains(fluid.getFluid())) mustUpdate = true;
            else oldFluids.remove(fluid.getFluid());
    ...
    public boolean pushPattern(ICraftingPatternDetails patternDetails, InventoryCrafting table) {
        if (this.returnStack != null) return false;
    ...
    public void readFromNBT(NBTTagCompound tagCompound) {
        ...
        if (tagCompound.hasKey("time")) this.ticksToFinish = tagCompound.getInteger("time");
        if (hasWorldObj()) {
    ...
    public void securityBreak() {
        if (this.getWorldObj() != null) getWorldObj().func_147480_a(this.xCoord, this.yCoord, this.zCoord, true);
    }

    @Override
    public void updateEntity() {
        if (getWorldObj() == null || getWorldObj().provider == null) return;
        if (this.ticksToFinish > 0) this.ticksToFinish = this.ticksToFinish - 1;
        if (this.ticksToFinish <= 0 && this.returnStack != null) {
            ...
            if (storage.getItemInventory().canAccept(toInject.copy())) {
                IAEItemStack nodAdded = storage.getItemInventory()
                        .injectItems(toInject.copy(), Actionable.SIMULATE, new MachineSource(this));
                if (nodAdded == null) {
    ...
    public void updateGrid(IGrid oldGrid, IGrid newGrid) {
        if (oldGrid != null) {
            ...
            if (storage != null) storage.getFluidInventory().removeListener(this);
        }
        if (newGrid != null) {
    ...
    public void writeToNBT(NBTTagCompound tagCompound) {
        ...
        else tagCompound.setBoolean("isContainerEmpty", true);
        if (this.returnStack != null) tagCompound.setTag("return", this.returnStack.writeToNBT(new NBTTagCompound()));
        else tagCompound.setBoolean("isReturnEmpty", true);
