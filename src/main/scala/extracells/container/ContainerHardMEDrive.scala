...
class ContainerHardMEDrive(
    inventory: InventoryPlayer,
    tile: TileEntityHardMeDrive
) extends Container {

  for (i <- 0 to 2) {
    addSlotToContainer(
      new SlotRespective(tile.getInventory, i, 80, 17 + i * 18) {
        override def isItemValid(item: ItemStack) =
          AEApi.instance.registries.cell().isCellHandled(item)
      }
    );
  }
  ...
  protected def bindPlayerInventory(): Unit = {
    for (i <- 0 to 2) {
      for (j <- 0 to 8) {
        addSlotToContainer(
          new Slot(inventory, j + i * 9 + 9, 8 + j * 18, 84 + i * 18)
        );
      }
    }
