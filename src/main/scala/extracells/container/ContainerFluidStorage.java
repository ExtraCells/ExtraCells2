...
public class ContainerFluidStorage extends Container
    ...
    protected void bindPlayerInventory(InventoryPlayer inventoryPlayer) {
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 9; j++) {
                addSlotToContainer(
                        new SlotPlayerInventory(inventoryPlayer, this, j + i * 9 + 9, 8 + j * 18, i * 18 + 122));
            }
        }

        for (int i = 0; i < 9; i++) {
            addSlotToContainer(new SlotPlayerInventory(inventoryPlayer, this, i, 8 + i * 18, 180));
        }
    }
        ...
        if (FluidUtil.isEmpty(container)) {
            ...
            if (fillSecondSlot(filledContainer.getRight())) {
                this.monitor.extractItems(
                        FluidUtil.createAEFluidStack(this.selectedFluid, filledContainer.getLeft()),
                        Actionable.MODULATE,
                        new PlayerSource(this.player, null));
                decreaseFirstSlot();
            }
    ...
    public void forceFluidUpdate(String searchText) {
        if (this.monitor != null) {
            ...
            new PacketFluidStorage(this.player, fluidStackList).sendPacketToPlayer(this.player);
            new PacketFluidStorage(this.player, this.hasWirelessTermHandler).sendPacketToPlayer(this.player);
        }
    }
    ...
    public void receiveSelectedFluid(Fluid _selectedFluid) {
        ...
        if (this.guiFluidStorage != null) this.guiFluidStorage.updateSelectedFluid();
    }
    ...
    public void updateFluidList(IItemList<IAEFluidStack> _fluidStackList) {
        ...
        if (this.guiFluidStorage != null) this.guiFluidStorage.updateFluids();
    }
