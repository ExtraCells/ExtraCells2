...
object UniversalTerminal {
  ...
  val terminals: Array[ItemStack] = {
    val terminals =
      new Array[ItemStack](if (isWcLLoaded) arrayLength - 1 else arrayLength)
    terminals.update(
      0,
    ...
    terminals.update(
      1,
      ItemEnum.PARTITEM.getDamagedStack(PartEnum.FLUIDTERMINAL.ordinal)
    )
    if (isMekLoaded) {
      terminals.update(
        2,
        ItemEnum.PARTITEM.getDamagedStack(PartEnum.GASTERMINAL.ordinal)
      )
      if (isThaLoaded)
