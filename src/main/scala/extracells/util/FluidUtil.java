...
public class FluidUtil {
     ...
     */
    public static MutablePair<ItemStack, FluidStack> fillItemFromAe(
            ItemStack container,
            FluidStack request,
        ...
        IAEFluidStack result =
                monitor.extractItems(FluidUtil.createAEFluidStack(request.getFluid(), requestAmount), mode, src);
        ...
        if (filledContainer.getLeft() < (int) result.getStackSize() && mode == Actionable.MODULATE) {
            ...
            IAEFluidStack notAdded = monitor.injectItems(
                    FluidUtil.createAEFluidStack(result.getFluid(), remaining), Actionable.MODULATE, src);
        ...
        return new MutablePair<>(
                filledContainer.getRight(), new FluidStack(request.getFluid(), filledContainer.getLeft()));
    }
     ...
     */
    public static ItemStack drainItemIntoAe(
            ItemStack container, IMEMonitor<IAEFluidStack> monitor, Actionable mode, BaseActionSource src) {
        if (container == null || container.stackSize != 1) {
            return null;
        }
        if (monitor == null) {
        ...
        if (partiallyDrained.getLeft() < fluidStack.amount) {
            ...
            if (mode == Actionable.MODULATE) {
                ...
                IAEFluidStack extracted = monitor.extractItems(
                        FluidUtil.createAEFluidStack(notAdded.getFluid(), duped), Actionable.MODULATE, src);
