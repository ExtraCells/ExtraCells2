...
public class FluidUtil {
     ...
     */
    public static MutablePair<ItemStack, FluidStack> fillItemFromAe(ItemStack container, FluidStack request,
            IMEMonitor<IAEFluidStack> monitor, Actionable mode, BaseActionSource src) {
        ...
        IAEFluidStack result = monitor
                .extractItems(FluidUtil.createAEFluidStack(request.getFluid(), requestAmount), mode, src);
        if (result == null || result.getStackSize() <= 0) {
            return null;
        }
        if (!result.getFluid().equals(request.getFluid())) {
            FMLLog.severe(
                    "[ExtraCells2] ME network returned fluid `%s` when requesting `%s`",
                    result.getFluid().getName(),
                    request.getUnlocalizedName());
            return null;
        }
        if (result.getStackSize() > (long) requestAmount) {
            FMLLog.severe(
                    "[ExtraCells2] ME network returned %d mB of fluid `%s` when requesting %d mB",
                    result.getStackSize(),
                    request.getUnlocalizedName(),
                    requestAmount);
            return null;
        ...
        if (filledContainer.getLeft() > (int) result.getStackSize()) {
            FMLLog.severe(
                    "[ExtraCells2] %d mB of fluid `%s` filled container `%s` with %d mB: check fluid handler implementation",
                    result.getStackSize(),
                    request.getUnlocalizedName(),
                    container.getUnlocalizedName(),
                    filledContainer.getLeft());
            return null;
        }
        if (filledContainer.getLeft() < (int) result.getStackSize() && mode == Actionable.MODULATE) {
            ...
            IAEFluidStack notAdded = monitor
                    .injectItems(FluidUtil.createAEFluidStack(result.getFluid(), remaining), Actionable.MODULATE, src);
            if (notAdded != null && notAdded.getStackSize() > 0) {
                FMLLog.severe(
                        "[ExtraCells2] %d mL of fluid `%s` voided when trying to fill container `%s`",
                        notAdded.getStackSize(),
                        notAdded.getFluid().getName(),
                        container.getUnlocalizedName());
            }
        }
        return new MutablePair<>(
                filledContainer.getRight(),
                new FluidStack(request.getFluid(), filledContainer.getLeft()));
    }
     ...
     */
    public static ItemStack drainItemIntoAe(ItemStack container, IMEMonitor<IAEFluidStack> monitor, Actionable mode,
        ...
        if (notAdded.getStackSize() > (long) fluidStack.amount) {
            FMLLog.severe(
                    "[ExtraCells2] ME network returned %d mB of fluid `%s` when injecting %d mB",
                    notAdded.getStackSize(),
                    fluidStack.getUnlocalizedName(),
                    fluidStack.amount);
            return null;
        ...
        if (partiallyDrained.getLeft() > fluidStack.amount) {
            FMLLog.severe(
                    "[ExtraCells2] Voided fluid: %d mB of fluid `%s` drained from container `%s` with %d mB: check fluid handler implementation",
                    notAdded.getStackSize(),
                    fluidStack.getUnlocalizedName(),
                    container.getUnlocalizedName(),
                    partiallyDrained.getLeft());
            // Void fluid, but at least don't void the container
            // This generally shouldn't happen, but might for certain configurations of many storage buses pointing at
            // the same almost-full tank
        ...
        if (partiallyDrained.getLeft() < fluidStack.amount) {
            ...
            if (mode == Actionable.MODULATE) {
                ...
                IAEFluidStack extracted = monitor.extractItems(
                        FluidUtil.createAEFluidStack(notAdded.getFluid(), duped),
                        Actionable.MODULATE,
                        src);
                if (extracted == null || extracted.getStackSize() < duped) {
                    FMLLog.severe(
                            "[ExtraCells2] %d mL of fluid `%s` duped when trying to empty container `%s`",
    ...
    public static boolean isFluidContainer(ItemStack itemStack) {
        if (itemStack == null) return false;
        Item item = itemStack.getItem();
        return item instanceof IFluidContainerItem || item == ItemEnum.FLUIDPATTERN.getItem()
    ...
    public static String formatFluidAmount(long millibuckets, boolean forceLongForm) {
        if (!forceLongForm) {
            if (millibuckets >= 1_000_000_000_000L) return String.format("%.1fG", millibuckets / 1_000_000_000_000.0);
            else if (millibuckets >= 1_000_000_000L) return String.format("%.1fM", millibuckets / 1_000_000_000.0);
            else if (millibuckets >= 1_000_000L) return String.format("%.1fk", millibuckets / 1_000_000.0);
    ...
    public static IItemList<IAEFluidStack> filterEmptyFluid(IItemList<IAEFluidStack> _fluidStackList) {
        ...
        for (IAEFluidStack fluid : _fluidStackList) {
            if (fluid.getStackSize() > 0) temp.add(fluid);
        }
        return temp;
    }
