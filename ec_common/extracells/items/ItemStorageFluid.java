...
public class ItemStorageFluid extends Item implements IStorageCell
{
	...
	{
		IItemList newPreformattedItems = Util.createItemList();

		if (Util.getCellRegistry().getHandlerForCell(itemstack).getPreformattedItems() != null)
		{
			for (ItemStack currentItemStack : Util.getCellRegistry().getHandlerForCell(itemstack).getPreformattedItems())
			{
				if (currentItemStack != null)
				{
					if (FluidContainerRegistry.isFilledContainer(currentItemStack))
					{
						newPreformattedItems.add(Util.createItemStack(new ItemStack(extracells.Extracells.FluidDisplay, 1, FluidContainerRegistry.getFluidForFilledItem(currentItemStack).getFluid().getID())));
					} else if (currentItemStack.getItem() == extracells.Extracells.FluidDisplay)
					{
						newPreformattedItems.add(Util.createItemStack(currentItemStack));
					}
				}
			}

			Util.getCellRegistry().getHandlerForCell(itemstack).setPreformattedItems(newPreformattedItems);
		}
		return itemstack;
	}
	...
	{
		// shall not save items, just storable in me drive
		return 1;
	}
