...
public class GUISolderingStation extends GuiScreen
{
	...
	public void drawScreen(int x, int y, float f)
	{
		...
		if (this.mc.thePlayer.getCurrentEquippedItem() != null && this.mc.thePlayer.getCurrentEquippedItem().getItem() == extracells.Cell && this.mc.thePlayer.getCurrentEquippedItem().getItemDamage() == 5)
		{
	...
	public void updateScreen()
	{
		...
		{
			ItemStack itemstack = this.mc.thePlayer.getCurrentEquippedItem();
			if (int_size != itemstack.getTagCompound().getInteger("costum_size"))
			{
				itemstack.getTagCompound().setInteger("costum_size", int_size);
			}

			if (int_types != itemstack.getTagCompound().getInteger("costum_types"))
			{
				itemstack.getTagCompound().setInteger("costum_types", int_types);
			}
			
			if (int_size != Integer.parseInt(textfield_size.getText()))
			{
				textfield_size.setText(Integer.toString(int_size));
			}

			if (int_types != Integer.parseInt(textfield_types.getText()))
			{
